...
Official repo: https://github.com/luodhhh/ModernTCN

A pure-convolutional backbone that revisits large-kernel depth-wise
convolutions with re-parameterisation and per-variable ConvFFN mixing over
patch embeddings. Adapted (self-contained: RevIN, series decomposition and the
flatten head are inlined) to the Time-Series-Library long-term-forecasting
interface.
"""
...
# ----------------------------------------------------------------------------
# Series decomposition + forecast head
# ----------------------------------------------------------------------------
...
class ModernTCN(nn.Module):
    def __init__(self, patch_size, patch_stride, stem_ratio, downsample_ratio, ffn_ratio, num_blocks, large_size,
                 small_size, dims, dw_dims, nvars, small_kernel_merged=False, backbone_dropout=0.1, head_dropout=0.1,
                 use_multi_scale=True, revin=True, affine=True, subtract_last=False, freq=None, seq_len=512, c_in=7,
                 individual=False, target_window=96):
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # --- hyper-parameters (robust getattr defaults; single-stage config) ---
        ...
        self.num_blocks = getattr(configs, 'num_blocks', [1])
        self.large_size = getattr(configs, 'large_size', [13])
        self.small_size = getattr(configs, 'small_size', [5])
        d = getattr(configs, 'd_model', 32)
        # dims / dw_dims need >=4 entries for the (fixed) down-sampling stems
        self.dims = getattr(configs, 'dims', [d, d, d, d])
        self.dw_dims = getattr(configs, 'dw_dims', [d, d, d, d])
        ...
        self.target_window = self.pred_len
        ...
        def _build():
            return ModernTCN(
                patch_size=self.patch_size, patch_stride=self.patch_stride, stem_ratio=self.stem_ratio,
                ...
                nvars=self.nvars, small_kernel_merged=self.small_kernel_merged, backbone_dropout=self.drop_backbone,
                head_dropout=self.drop_head, use_multi_scale=self.use_multi_scale, revin=self.revin,
                affine=self.affine, subtract_last=self.subtract_last, freq=self.freq, seq_len=self.seq_len,
        ...
        if self.decomposition:
            ...
            self.model_res = _build()
            self.model_trend = _build()
        else:
            self.model = _build()
